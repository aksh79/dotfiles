...
#+--- Bars LEFT ---+
# Session name
tmux set -g status-left "#[fg=${THEME[bblack]},bg=${THEME[red]},bold] #{?client_prefix,p ,#[dim]󰣇 `hostname` }#[bold,nodim][#S] "

#+--- Windows ---+
# Focus
tmux set -g window-status-current-format "$RESET#[fg=${THEME[green]},bg=${THEME[bblack]}] #{?#{==:#{pane_current_command},ssh},󰣀 , }#[fg=${THEME[foreground]},bold,nodim]$window_number#W#[nobold]#{?window_zoomed_flag, $zoom_number, $custom_pane}"
# Unfocused
tmux set -g window-status-format "$RESET#[fg=${THEME[foreground]}, bg=default] #{?#{==:#{pane_current_command},ssh},󰣀 , }$window_number#W#[nobold,dim]#{?window_zoomed_flag, $zoom_number, $custom_pane}#[fg=${THEME[yellow]}]"
